...
function Convert-Report {
    ...
    try {
        ...
        if ($headerRow -gt 0) {
            ...
            # Filter numeric cols based on first data row
            $validCols = @()
            foreach ($c in $amountCols) {
                $sampleVal = $sh.Cells.Item($startRow, $c).Value2
                if ($sampleVal -ne $null) {
                     $d = 0.0
                     if ($sampleVal -is [System.ValueType] -or ($sampleVal -is [string] -and [double]::TryParse($sampleVal, [ref]$d))) {
                         $validCols += $c
                     }
                }
            }
                ...
                for ($r = $startRow; $r -le $lastRow; $r++) {
                    foreach ($c in $validCols) {
                        ...
                        if ($val -is [System.ValueType]) {
                            # Safely cast to double (e.g. from int/double COM object)
                            try { $dVal = [double]$val; $canConvert = $true } catch {}
                        }
