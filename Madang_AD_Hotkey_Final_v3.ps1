...
function Convert-NameToId {
    ...
    $results = try {
        $adResult = powershell.exe -ExecutionPolicy Bypass -File $Global:SearchADPath "$name"
        if ($adResult) {
            $adResult | ConvertFrom-Json -ErrorAction Stop
        } else {
            $null
        }
    } catch {
...
try {
    ...
    $timer.Interval = 500
    $timer.Tag = [pscustomobject]@{ Form = $null }
    $timer.add_Tick({
        $_.Source.Stop()
        $_.Source.Tag.Form = [System.Windows.Forms.MessageBox]::Show("마당쇠 실시간 변환기가 시작되었습니다.`n`n사용법:`n1. 변환할 이름을 마우스로 드래그하여 선택`n2. Ctrl + Shift + N 키를 누릅니다.`n`n(백그라운드에서 조용히 실행됩니다)", "마당쇠 알림", 0, [System.Windows.Forms.MessageBoxIcon]::Information)
    })
    $timer.Start()
