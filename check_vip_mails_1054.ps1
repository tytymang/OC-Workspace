try {
    ...
    # Build VIP names via Unicode code points
    ...
    foreach ($item in $unreadItems) {
        if ($item.ReceivedTime -gt (Get-Date).AddMinutes(-40)) {
            $isVip = $false
    ...
    if ($results.Count -eq 0) {
        "[]" | Out-File -FilePath "C:\Users\307984\.openclaw\workspace\hb_vip_mails_1054.json" -Encoding UTF8
    } else {
        $results | ConvertTo-Json | Out-File -FilePath "C:\Users\307984\.openclaw\workspace\hb_vip_mails_1054.json" -Encoding UTF8
    }
} catch {
    $_.Exception.Message | Out-File -FilePath "C:\Users\307984\.openclaw\workspace\hb_mail_err_1054.txt"
}
