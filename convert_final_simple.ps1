    ...
    try {
        ...
        for ($r=1; $r -le 20; $r++) {
            for ($c=1; $c -le 20; $c++) {
                ...
                if ($v -ne $null -and $v.ToString().Contains($targetStr)) {
                    $newH = $v.ToString().Replace($targetStr, "M$")
                    $sh.Cells.Item($r, $c).Value2 = $newH
                    $cols += $c
                    $headerRow = $r
                }
            }
        ...
        if ($headerRow -gt 0) {
            ...
            $validCols = @()
            foreach ($c in $cols) {
                $sv = $sh.Cells.Item($startRow, $c).Value2
                if ($sv -ne $null) {
                     # Just assume valid if not null for now to avoid type errors in checking
                     $validCols += $c
                }
            }
            ...
            if ($validCols.Count -gt 0) {
                ...
                for ($r = $startRow; $r -le $lastRow; $r++) {
                    foreach ($c in $validCols) {
                        ...
                        if ($val -ne $null) {
                            # Force PS to treat as double if possible
                            $dVal = $val -as [double]
                            if ($dVal -ne $null) {
                                # Calc: (Val * 100,000,000) / Rate / 1,000,000
