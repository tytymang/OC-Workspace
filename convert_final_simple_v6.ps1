...
function Convert-One-File {
    ...
    try {
            ...
            if ($cols.Count -gt 0) {
                ...
                for ($r = $startRow; $r -le $lastRow; $r++) {
                    foreach ($c in $cols) {
                        ...
                        if ($val -ne $null) {
                            # Manual parse without tryparse, just rely on type
                            # And direct COM assignment
                            try {
                                # Assume it's double if value type
        ...
        # Save logic
        # Save as temp file in same dir
        $dir = [System.IO.Path]::GetDirectoryName($Path)
        $tempName = [System.IO.Path]::Combine($dir, "temp_report_" + [System.Guid]::NewGuid().ToString() + ".xlsx")
