...
out_dir = r"C:\Users\307984\.openclaw\workspace\temp_attachments\clipboard_images"
...
for i, slide in enumerate(presentation.Slides):
    for j, shape in enumerate(slide.Shapes):
        if shape.Type == 13 or shape.Type == 11:  # Picture
            shape.Copy()
            time.sleep(0.5)
            try:
                img = ImageGrab.grabclipboard()
                if img:
                    img.save(os.path.join(out_dir, f"slide_{i+1}_img.png"), "PNG")
