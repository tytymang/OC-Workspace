...
try {
    ...
    for ($r=1; $r -le 10; $r++) {
        for ($c=1; $c -le 20; $c++) {
            ...
            if ($val -match $searchHeader) {
                # 헤더 후보 발견. 하지만 이게 진짜 컬럼 헤더인지 확인 필요.
                # 보통 이름 컬럼과 같은 행에 있어야 함.
                # 일단 저장해두고 계속 검색해볼 수도 있음.
                $headerRow = $r
                $headerCol = $c
    ...
    # 만약 값이 비어있다면, 혹시 병합된 셀(Merged Cell) 문제일 수 있음.
    # 또는 헤더가 2줄 이상인 경우(병합된 헤더)일 수 있음.
