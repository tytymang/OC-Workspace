...
try {
    ...
    foreach ($sheet in $workbook.Sheets) {
        ...
        if ($foundCell) {
            $row = $foundCell.Row
            Write-Host "Target FOUND at Row $row"
            ...
            Write-Host "Cols: KRW=$krwCol, USD=$usdCol"
            ...
            if ($krwVal -is [double] -or $krwVal -is [int]) {
                ...
                Write-Host "Original KRW: $krwVal"
                Write-Host "New USD: $usdVal"
                
                if ($usdCol -gt 0) {
                    $sheet.Cells.Item($row, $usdCol).Value2 = $usdVal
                    Write-Host "Updated USD Cell"
                } else {
                    $sheet.Cells.Item($row, $krwCol + 1).Value2 = $usdVal
                    Write-Host "Wrote to Right Cell"
                }
                $found = $true
            } else {
                Write-Warning "KRW value is not a number"
            }
            break # 한 명만 수정
        }
    }
