...
# VIP Names (Unicode)
$v1 = -join @([char]0xC774, [char]0xC815, [char]0xD6C8) # 이정훈
$v2 = -join @([char]0xC774, [char]0xC815, [char]0xC6B0) # 이정우
$v3 = -join @([char]0xC774, [char]0xC0C1, [char]0xBB34) # 이상무
$v4 = -join @([char]0xC774, [char]0xC601, [char]0xC91C) # 이영주
$vips = @($v1, $v2, $v3, $v4)

function Check-Folder($f) {
    try {
        # Check unread since 12:00 PM
        $items = $f.Items.Restrict("[UnRead] = true AND [ReceivedTime] > '2026-03-25 12:00'")
        foreach ($i in $items) {
            $sender = try { $i.SenderName } catch { "" }
            foreach ($v in $vips) {
                if ($sender -like "*$v*") {
                    $results.emails += @{ Sender = $sender; Subject = $i.Subject }
                    break
                }
            }
        }
