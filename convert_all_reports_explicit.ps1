        ...
        if ($headerRow -gt 0) {
            ...
            foreach ($c in $amountCols) {
                ...
                if ($sampleVal -ne $null) {
                     # Simple check using -is [System.ValueType] again as TryParse seems buggy via COM in this env
                     if ($sampleVal -is [System.ValueType]) {
                         $validCols += $c
                     }
                     # Else check if it's a string that looks like a number
                     elseif ($sampleVal -is [string] -and $sampleVal -match "^[\d\.]+$") {
                         $validCols += $c
                     }
                }
            }
